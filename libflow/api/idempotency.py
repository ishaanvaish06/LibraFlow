...
class IdempotencyStore:
    ...
    def set_in_progress(self, key: str, timeout_seconds: int = 60) -> bool:
        full_key = f"idemp:{key}"
        if self.redis_client is not None:
            try:
    ...
    def set_completed(self, key: str, status_code: int, response_body: Dict[str, Any]) -> None:
        full_key = f"idemp:{key}"
        record = {
            "status": "COMPLETED",
...
def handle_idempotent_operation(
    ...
    acquired = store.set_in_progress(idempotency_key)
    if not acquired:
        raise HTTPException(
            status_code=409,
            detail=f"Concurrent operation with Idempotency-Key '{idempotency_key}' is currently in progress."
    ...
    try:
        result = operation_fn()
        store.set_completed(idempotency_key, status_code, result)
        return result
    except Exception:
        store.clear(idempotency_key)
        raise
